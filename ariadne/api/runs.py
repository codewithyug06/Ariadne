...
async def list_runs(
    ...
    runs, total = await recorder.list_runs(limit=limit, offset=offset)
...
async def get_run(session_id: str, request: Request) -> RunDetailResponse:
    recorder = request.app.state.recorder
    await recorder.flush()
    run = await recorder.get_run(session_id)
    if run is None:
        raise HTTPException(status_code=404, detail=f"no run recorded for session {session_id!r}")
    events = await recorder.get_events(session_id)
...
async def get_run_graph(session_id: str, request: Request) -> SessionGraph:
    graph: SessionGraph = await request.app.state.graph_builder.session_graph(session_id)
    if not graph.nodes:
...
async def get_root_cause(
    ...
    chain = await request.app.state.graph_builder.blame_chain(node_id, max_depth)
    if not chain:
...
async def get_blast_radius(
    ...
    nodes = await request.app.state.graph_builder.blast_radius(node_id, max_depth)
